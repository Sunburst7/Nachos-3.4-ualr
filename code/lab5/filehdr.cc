...
void FileHeader::Print()
{
	...
	{
		printf("print second content start");

		int dataSectors2[NumDirect2];
		synchDisk->ReadSector(dataSectors[lastIndex], (char *)dataSectors2);
		printf("FileHeaderd contents.  File size: %d.  File blocks:\n", numBytes);
		...
		for (i = k = 0; i < lastIndex; i++)
		{
			...
			// printf("print second content middle");
			// printf("\n");
		}

		// printf("print second content middle");
		// printf("numSectors %d", numSectors);

		i = 0;
		for (; i < numSectors - lastIndex; i++)
		{
		...
		printf("print second content end");
	}
	delete[] data;
}
...
// 		synchDisk->WriteSector(dataSectors[lastIndex], (char *)dataSectors2);
// 	}
// 	freeMap->WriteBack(bitMapHeader);
// 	return true;
// }
