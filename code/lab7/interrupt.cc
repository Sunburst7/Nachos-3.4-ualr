...
{
    //	在程序执行结束后打印TLB信息
    printf("Page Miss: %d, Page Hit: %d, Total Translate: %d, Page Hit Rate: %.2lf%%\n",
           machine->TranslateCount-machine->PageHitCount,
           machine->PageHitCount,
           machine->TranslateCount,
...
void InitProcess(int spaceId) {// 新线程执行用户进程初始化操作
...
void Interrupt::Exec() {
    ...
    printf("Exec(%s)\n",filename);
    AddrSpace *space = new AddrSpace(filename);// 建立新地址空间
...
void Interrupt::PageFault(int badVAddr) {
    ...
    int newPage = badVAddr / PageSize; // 虚拟页表中页.valid=FALSE
